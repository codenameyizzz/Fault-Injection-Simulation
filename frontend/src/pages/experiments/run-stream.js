...
export default function RunStreamPage() {
  ...
  useEffect(() => {
    ...
    const es = new EventSource(
      `${apiUrl}/experiments/run-stream?fault_type=${ft}&start_pct=${sp.get(
        "start_pct"
      )}&step_pct=${sp.get("step_pct")}&end_pct=${sp.get(
        "end_pct"
      )}&disk_id=${sp.get("disk_id")}&slow_io_count=${sp.get(
        "slow_io_count"
      )}&delay_min_us=${sp.get("delay_min_us")}&delay_max_us=${sp.get(
        "delay_max_us"
      )}&label=${lbl}`
    );
    ...
    es.onmessage = (e) => {
      ...
      if (e.data.startsWith("Process exited")) {
        es.close();
        fetch(
          `${apiUrl}/experiments/analyze-sweep?fault_type=${ft}&label=${lbl}`
        )
          .then((r) => {
          ...
          .catch((_) => setLines((l) => [...l, "[Error fetching chart]"]));
      }
    };
