// frontend/src/pages/Jonathan/firmware_throttle/results.js
...
export default function FirmwareThrottleResults() {
  const sp = useSearchParams();
  const router = useRouter();
  const fault = sp.get("fault"); // "firmware_throttle"
  ...
  if (!param && runs) {
    ...
    return (
      <div className="container py-4">
        <h3>Select parameter sweep for firmware throttle</h3>
          ...
          {params.map((p) => (
            <li key={p} className="list-group-item">
              <button
                ...
                onClick={() =>
                  router.push(
                    `/Jonathan/firmware_throttle/results?fault=${fault}&param=${p}`
                  )
                }
              >
                {p}
          ...
          {labels.map((lbl) => (
            <li key={lbl} className="list-group-item">
              <button
                ...
                onClick={() =>
                  router.push(
                    `/Jonathan/firmware_throttle/results?fault=${fault}&param=${param}&label=${lbl}`
                  )
                }
              >
                {lbl}
