// frontend/src/pages/Jonathan/firmware_bandwidth_drop/results.js
"use client";

import { useSearchParams, useRouter } from "next/navigation";
import { useEffect, useState } from "react";

export default function FirmwareBandwidthDropResults() {
  const sp = useSearchParams();
  const router = useRouter();

  const fault = sp.get("fault"); // "firmware_bandwidth_drop"
  ...
  if (!param && runs) {
    ...
    return (
      <div className="container py-4">
        <h3>Select parameter for Firmware BW Drop</h3>
          ...
          {params.map((p) => (
            <li key={p} className="list-group-item">
              <button
                ...
                onClick={() =>
                  router.push(
                    `/Jonathan/firmware_bandwidth_drop/results?fault=${fault}&param=${p}`
                  )
                }
              >
                {p}
          ...
          {labels.map((lbl) => (
            <li key={lbl} className="list-group-item">
              <button
                ...
                onClick={() =>
                  router.push(
                    `/Jonathan/firmware_bandwidth_drop/results?fault=${fault}&param=${param}&label=${lbl}`
                  )
                }
              >
                {lbl}
  ...
  if (param && label) {
    return (
      <div className="container py-4">
        <h3>
          CDF Plot: Firmware BW Drop / <code>{param}</code> (run {label})
        </h3>
        {imgUrl ? (
          <>
